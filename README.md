...
| `'X' was not declared in this scope` for a global | The Arduino preprocessor forward-declares **functions** only, never variables. A global `#define`, `constexpr` or variable must appear above every function that uses it, and outside any `#if` narrower than its users. Hit on 2026-09-14 by `TIMING_MAX_PLAUSIBLE_US` |
| `Multiple libraries were found for "SD.h"` | Harmless here — both copies are 1.3.0 and the sketchbook one (`Documents\Arduino\libraries\SD`) wins, which is the pinned version. Delete that folder only if you want the IDE's bundled copy to be used instead |
| Bench DC on A14 looks like ~0 mA after filtering | `current_filter_test.py` removes the idle offset, and on a flat calibration capture the offset **is** the signal. Read the `absolute level` line in the INPUT block, or pass `--no-baseline`. A 1.100 V input should read ~9137 counts = ~110.0 mA |
| Battery logs a constant 3.95 V | That is `adc_max × vref × div_ratio` — the ADC is pinned, so the value is a floor, not a reading. The supply is reaching the **ADC pad** instead of the divider input, or a divider resistor is open. The pad must stay under ~1.98 V; 3.3 V at the divider input gives ~1.65 V at the pad ≈ 13 686 counts |
| Battery voltage is plausible-looking but wrong by a constant factor | `div_ratio` is **compiled into the firmware**, not measured. If the divider was rebuilt, update `BATTERY_R_TOP_OHM` / `BATTERY_R_BOTTOM_OHM` and reflash — the parser uses the ratio the log carries |
